...
**OAuth 2.0 管授权**：让第三方应用代替用户访问资源，但不负责证明用户是谁。  
**OIDC 管登录**：在 OAuth 2.0 上增加身份层，用 ID Token 说明用户是谁。  
**SAML 2.0 管企业 SSO**：身份提供方通过 XML 断言，把认证结果传给其他系统。  
**Kerberos 靠票据认证**：用户登录一次后凭票访问内网服务，密码无需反复传输。  
**LDAP 负责查目录**：集中存储和查询用户、组织等信息，常作为认证的数据来源。  

记忆主线：**OAuth 给权限，OIDC 认身份，SAML 做联邦，Kerberos 发票据，LDAP 查用户。**
...
- **登录（Login）**：认证成功后，为当前客户端建立可持续使用的登录状态。
- **授权（Authorization）**：收到资源请求时，判断“你能做什么”，通常会在登录后的每次请求中反复发生。

完整链路可以拆成五步：

1. **身份声明**：用户先声明账号或身份标识。
2. **选择并执行认证**：业务应用可以直接使用密码、MFA、WebAuthn、Kerberos 等机制认证，也可以通过 OIDC 或 SAML 把认证委托给 IdP。后一种情况下，IdP 内部仍会使用密码、MFA、WebAuthn、Kerberos、LDAP/AD 等方式核验用户。
3. **接收并验证认证结果**：应用验证本地认证上下文，或验证 IdP 返回的 OIDC ID Token、SAML Assertion 等认证产物。
4. **建立登录态**：应用签发 Session Cookie，或向客户端提供 Access Token 与 Refresh Token，让后续请求不必重新认证。
5. **授权访问**：服务端结合 Session/Token 与 RBAC、ABAC、ACL 等规则，对每次资源请求作出允许或拒绝的决定。

![登录、认证、授权与协议协作关系](authentication-authorization-flow.svg)

#### 替换关系与搭配关系

| 关系 | 技术 | 如何理解 |
| --- | --- | --- |
| 通常替换 | **OIDC ↔ SAML** | 都是面向业务应用的认证与身份联邦协议，都能实现跨系统 SSO；OIDC 更适合现代 Web、App 和 API，SAML 常见于传统企业 SaaS。 |
| 按架构替换或并存 | **Session Cookie ↔ Access/Refresh Token** | 都可维持登录并支撑后续请求；传统服务端应用常用 Session，SPA、App、API 常用 Token 组合。 |
| 按环境选择 | **密码/MFA/WebAuthn ↔ Kerberos** | 都属于实际核验用户身份的机制；Kerberos 主要服务于受控内网和域环境。 |
| 典型搭配 | **OAuth 2.0 + OIDC** | OAuth 负责授权并签发 Access Token，OIDC 增加身份认证并签发 ID Token。 |
| 典型搭配 | **LDAP/AD + Kerberos** | LDAP/AD 查询用户、组和属性，Kerberos 通过票据完成内网认证。 |
| 典型搭配 | **LDAP/AD + IdP + OIDC/SAML** | LDAP/AD 作为身份数据源，IdP 完成认证，再用 OIDC 或 SAML 向外部应用传递结果。 |
| 必须搭配 | **登录协议 + RBAC/ABAC/ACL** | 协议证明用户身份或携带凭证，业务权限模型才决定具体资源是否放行。 |

> **JWT 不是登录或授权协议，而是一种 Token 格式。** OIDC 的 ID Token 通常是 JWT；OAuth 的 Access Token 可以是 JWT，也可以是不透明字符串。
...
![img](https://cdn.jsdelivr.net/gh/Supremes/blog-images@master/imgs/articles/OAuth.webp)
...
![img](https://cdn.jsdelivr.net/gh/Supremes/blog-images@master/imgs/articles/OIDC.webp)
...
![img](https://cdn.jsdelivr.net/gh/Supremes/blog-images@master/imgs/articles/SAML.webp)
...
![img](https://cdn.jsdelivr.net/gh/Supremes/blog-images@master/imgs/articles/Kerberos.webp)
