...
summary: A deep dive into what Anthropic, OpenAI, Perplexity and LangChain are actually building. Covering the orchestration loop, tools, memory, con...
tags: []
---
...
**类型：** 技术长文 / 深度分析｜**116万浏览，2427 喜欢**

### TL;DR

对 Anthropic、OpenAI、LangChain、CrewAI 等主流 AI 框架的 **Agent Harness（智能体底座）** 进行深度解析。核心论点：模型本身不是瓶颈，包裹模型的基础设施才是。

**关键证据：** LangChain 仅改变基础设施（不换模型），在 TerminalBench 2.0 从 30 名开外跃升至第 5；另有研究显示让 LLM 优化自身 harness 后达到 76.4% 通过率。

**经典公式（LangChain Vivek Trivedy）：** *"If you're not the model, you're the harness."*

### 生产级 Harness 的 12 个核心组件

| # | 组件 | 关键细节 |
|---|------|----------|
| 1 | **编排循环** | TAO/ReAct 循环，Anthropic 称其为"dumb loop" |
| 2 | **工具层** | 注册、Schema 校验、沙箱执行 |
| 3 | **内存** | 短期（会话内）+ 长期（CLAUDE.md / MEMORY.md） |
| 4 | **上下文管理** | 中间位置内容性能下降 30%+（"Lost in the Middle"），策略：压缩 / 观测遮蔽 / JIT 检索 |
| 5 | **Prompt 构建** | 层级化：系统提示 → 工具定义 → 记忆文件 → 对话历史 |
| 6 | **输出解析** | 依赖原生 tool_calls 结构化输出，淘汰文本解析 |
| 7 | **状态管理** | Claude Code：git commit 作为检查点；LangGraph：类型字典 + 节点 |
| 8 | **错误处理** | 10 步流程每步 99% 成功率 → 整体仅 90.4%；分 4 类处理 |
| 9 | **护栏与安全** | OpenAI：输入 / 输出 / 工具三层；Claude Code：~40 个工具能力独立门控 |
| 10 | **验证循环** | 给模型验证能力可提升质量 2-3x（Boris Cherny） |
| 11 | **子智能体编排** | Fork / Teammate / Worktree 三种模式 |
| 12 | **（长任务）Ralph Loop** | Initializer Agent 建环境 → Coding Agent 跨 context window 通过 git / 进度文件持续 |

### 7 大架构决策

1. **单 Agent vs 多 Agent** — 工具数 <10 优先单 Agent
2. **ReAct vs Plan-and-Execute** — LLMCompiler 报告 3.6x 加速
3. **上下文窗口策略** — ACON 研究：减少 26-54% Token，保留 95%+ 精度
4. **验证循环设计** — 确定性验证 vs LLM-as-judge
5. **权限与安全架构** — 宽松（快但有风险）vs 严格（慢但安全）
6. **工具精简** — Vercel 删除 80% 工具效果反而更好；Claude Code 懒加载减少 95% 上下文
7. **Harness 厚度** — Anthropic 押注薄 harness + 模型提升；图框架押注显式控制

### Actionable Takeaways

- Agent 失败时，先查 harness，不要怪模型
- 优化上下文管理（位置、压缩、懒加载）往往比换模型更有效
- 验证循环是 demo 与生产级 Agent 的最大区别
- Harness 设计的健康检测：换更强模型后，若不需要增加 harness 复杂度，设计就是合理的
- 工具不是越多越好，最小化暴露工具集